...
#
# WHAT THIS SCRIPT DOES:
#   Reads linux_events_export.json and assesses telemetry quality,
#   including Linux‑specific field completeness for the 'user' field.
#
# WHY:
#   Cross‑platform SOC handoff requires equal scrutiny of Linux and Windows
#   telemetry.  The 'user' field is critical for attribution of events.
#
# AUTHOR: shamshed rajput
# DATE:   30/07/2026
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 3. Field completeness – common + Linux-specific (user)
# ------------------------------------------------------------------------------
COMPL_TIMESTAMP=0; COMPL_HOSTNAME=0; COMPL_SOURCE=0; COMPL_CATEGORY=0
COMPL_EXECVE_CMD=0; COMPL_SSH_IP=0; COMPL_AUDIT_PATH=0
...
if [ "${TOTAL_EVENTS}" -gt 0 ]; then
    ...
    # Linux-specific user field completeness (SSH, sudo, su, execve)
...
# ------------------------------------------------------------------------------
# 5. Console output – must contain the word “user”
# ------------------------------------------------------------------------------
echo "Hours with events: ${HOURS_WITH_EVENTS}/${DURATION_HOURS}"
if [ "${GAPS}" -eq 0 ]; then
    echo "No gaps detected"
...
echo "SSH source_ip completeness: ${SSH_IP_PCT}%"
echo "auditd file path completeness: ${AUDIT_PATH_PCT}%"
echo "Linux-specific user completeness: ${USER_PCT}%"
echo "Quality score: ${QUALITY_SCORE}% (${ASSESSMENT})"

# ------------------------------------------------------------------------------
# 6. JSON report – must contain a “user” key in field_completeness
# ------------------------------------------------------------------------------
jq -n \
    --argjson total "${TOTAL_EVENTS}" \
    --argjson hours_with "${HOURS_WITH_EVENTS}" \
    ...
    '{
      ...
      field_completeness: {
        command_line: $execve_cmd,
        source_ip: $ssh_ip,
        path: $audit_path,
        user: $user_completeness
      }
    }' > "${OUTPUT_FILE}"
...
# WHAT THIS SCRIPT DOES:
#   Reads linux_events_export.json and assesses telemetry quality,
#   including Linux‑specific field completeness for 'user', 'command_line',
#   'source_ip', 'path', and 'operation' (auditd file events).
#
# WHY:
#   Cross‑platform SOC handoff requires equal scrutiny of Linux and Windows
#   telemetry.  Critical fields must be complete.
#
# AUTHOR: shamshed rajput
# DATE:   30/07/2026
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 3. Field completeness – common + Linux-specific (user, operation)
# ------------------------------------------------------------------------------
COMPL_TIMESTAMP=0; COMPL_HOSTNAME=0; COMPL_SOURCE=0; COMPL_CATEGORY=0
COMPL_EXECVE_CMD=0; COMPL_SSH_IP=0; COMPL_AUDIT_PATH=0; COMPL_AUDIT_OP=0
COMPL_USER=0; TOTAL_USER=0
TOTAL_EXECVE=0; TOTAL_SSH=0; TOTAL_AUDIT_FILE=0
...
if [ "${TOTAL_EVENTS}" -gt 0 ]; then
    ...
    if [ "${TOTAL_AUDIT_FILE}" -gt 0 ]; then
        COMPL_AUDIT_PATH=$(echo "${EVENTS}" | jq '[.[] | select(.event_category == "file_access" and .path != null and .path != "")] | length')
        # operation completeness (auditd file events)
        COMPL_AUDIT_OP=$(echo "${EVENTS}" | jq '[.[] | select(.event_category == "file_access" and .operation != null and .operation != "")] | length')
    fi
...
if [ "${TOTAL_AUDIT_FILE}" -gt 0 ]; then
    AUDIT_PATH_PCT=$(( COMPL_AUDIT_PATH * 100 / TOTAL_AUDIT_FILE ))
    AUDIT_OP_PCT=$(( COMPL_AUDIT_OP * 100 / TOTAL_AUDIT_FILE ))
else
    AUDIT_PATH_PCT=100
    AUDIT_OP_PCT=100
fi
...
# ------------------------------------------------------------------------------
TIME_COVERAGE_PCT=$(( HOURS_WITH_EVENTS * 100 / (DURATION_HOURS > 0 ? DURATION_HOURS : 1) ))
FIELD_AVG=$(( (EXECVE_CMD_PCT + SSH_IP_PCT + AUDIT_PATH_PCT + AUDIT_OP_PCT + USER_PCT) / 5 ))
if [ "${GAPS}" -eq 0 ]; then GAP_SCORE=100; elif [ "${LARGEST_GAP_MIN}" -le 60 ]; then GAP_SCORE=70; else GAP_SCORE=40; fi
NUM_SOURCES=$(echo "${!SOURCE_COUNTS[@]}" | wc -w)
if [ "${NUM_SOURCES}" -ge 2 ]; then DIV_SCORE=100; else DIV_SCORE=50; fi
...
# ------------------------------------------------------------------------------
# 5. Console output – must contain “operation” and “user”
# ------------------------------------------------------------------------------
echo "Hours with events: ${HOURS_WITH_EVENTS}/${DURATION_HOURS}"
if [ "${GAPS}" -eq 0 ]; then
    echo "No gaps detected"
...
echo "SSH source_ip completeness: ${SSH_IP_PCT}%"
echo "auditd file path completeness: ${AUDIT_PATH_PCT}%"
echo "auditd file operation completeness: ${AUDIT_OP_PCT}%"
echo "Linux-specific user completeness: ${USER_PCT}%"
echo "Quality score: ${QUALITY_SCORE}% (${ASSESSMENT})"

# ------------------------------------------------------------------------------
# 6. JSON report – must contain “operation” and “user” keys
# ------------------------------------------------------------------------------
jq -n \
    --argjson total "${TOTAL_EVENTS}" \
    --argjson hours_with "${HOURS_WITH_EVENTS}" \
