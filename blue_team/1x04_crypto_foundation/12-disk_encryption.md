...
| |
| RECOMMENDATION: NAS-01 should use full-disk encryption with LUKS. |
| full-disk encryption ensures that all backup data, including PHI, is |
| protected at rest. This prevents data exposure if the NAS is stolen or |
| compromised through the flat network. |
| |
| Three encryption levels were considered for NAS-01: |
| |
| 1) FULL-DISK ENCRYPTION: Encrypts the entire disk including OS |
| - PROS: Simple, protects everything |
| - CONS: Encrypts unnecessary data, performance impact on NAS OS |
| - NOT RECOMMENDED for NAS-01 (encrypts OS data unnecessarily) |
| |
| 2) VOLUME-LEVEL ENCRYPTION (LUKS): Encrypts only the backup partition |
| - PROS: Industry standard, good performance, selective encryption |
| - CONS: Requires manual key management |
| - RECOMMENDED for NAS-01 |
| |
| 3) FILE-LEVEL ENCRYPTION: Encrypts individual files (Veeam native) |
...
| - full-disk encryption would encrypt the entire NAS OS unnecessarily |
...
================================================================================
    ...
    meddefense-crypto-audit-notes.txt (NAS-01 plaintext)

    1x00 Asset Registry (NAS-01)

    1x01 Kill Chains (Ransomware targets backups)

    1x02 Finding 015 (NAS management interface accessible)

    1x03 Strategy (Offsite backup replication)

    NIST SP 800-111: Guide to Storage Encryption

================================================================================
END OF DISK ENCRYPTION LAB REPORT
================================================================================
