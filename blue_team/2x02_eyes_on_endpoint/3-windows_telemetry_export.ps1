...
foreach ($Source in $LogSources) {
    ...
    if ($Events) {
        ...
        foreach ($Event in $Events) {
            ...
            # Enrich specific Event IDs
            # For 4624/4625, TargetUserName is the primary account field in the event XML
            switch ($Event.Id) {
                4624 {
                    # TargetUserName (the account that logged on)
                ...
                4625 {
                    # TargetUserName (the account that failed to log on)
                    if ($Event.Message -match "Account Name:\s+(.*)") { $Normalized.enrichment.TargetUserName = $Matches[1].Trim() }
                    if ($Event.Message -match "Failure Reason:\s+(.*)") { $Normalized.enrichment.failure_reason = $Matches[1].Trim() }
                    if ($Event.Message -match "Source Network Address:\s+(.*)") { $Normalized.enrichment.source_ip = $Matches[1].Trim() }
                    $Normalized.event_category = "Failed Logon"
