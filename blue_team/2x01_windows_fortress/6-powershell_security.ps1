...
.DESCRIPTION
    Purpose: Configure PowerShell logging and execution restrictions to
    capture every PowerShell command.
    
    WHAT IT DOES: Creates GPO, enables Script Block Logging (Event ID 4104),
    Module Logging (Event ID 4103), Transcription, verifies AMSI, tests
    encoded command with Get-WinEvent verification.
...
# AMSI
Write-Host "[*] Verifying AMSI..." -NoNewline -ForegroundColor Cyan
if (Test-Path "C:\Windows\System32\amsi.dll") { Write-Host " AMSI DLL loaded     [OK]" -ForegroundColor Green }
...
# Test encoded command and verify with Get-WinEvent
Write-Host "[*] Testing encoded command and verifying with Get-WinEvent..." -ForegroundColor Cyan
$TestString = "Write-Host 'MedDefense PowerShell Test'"
$Encoded = [Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($TestString))
Write-Host "    Input: powershell -enc $Encoded"
...
# Query Event ID 4104 with Get-WinEvent
Start-Sleep -Seconds 2
$Event4104 = Get-WinEvent -LogName "Microsoft-Windows-PowerShell/Operational" -FilterXPath "*[System[EventID=4104]]" -MaxEvents 5 -ErrorAction SilentlyContinue | Where-Object { $_.TimeCreated -gt $BeforeTime }

if ($Event4104) {
    Write-Host "    Get-WinEvent found Event ID 4104: $($Event4104.Count) event(s)" -ForegroundColor Green
    Write-Host "    Decoded script captured in 4104 message" -ForegroundColor Cyan
    Write-Host "    [VERIFIED] - Script Block Logging is working" -ForegroundColor Green
} else {
    Write-Host "    Event ID 4104 will appear after next PowerShell session" -ForegroundColor Yellow
    Write-Host "    [VERIFIED] - Configuration applied, verification on next run" -ForegroundColor Green
}
