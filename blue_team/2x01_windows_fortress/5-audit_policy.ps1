...
.DESCRIPTION
    Purpose: Configure Advanced Audit Policies via GPO.
    WHAT IT DOES: Creates GPO, 7 audit categories, ProcessCreationIncludeCmdLine_Enabled,
    restrict Security log Clear, Security log max size 1073741824, links, verifies.
...
Write-Host "[*] Configuring Audit Categories..." -ForegroundColor Cyan
$AuditCategories = @(
    @{Sub="Credential Validation"; S="1"; F="1"; D="Credential Validation"}
    @{Sub="Kerberos Authentication"; S="1"; F="1"; D="Kerberos Authentication"}
    @{Sub="Logon"; S="1"; F="1"; D="Logon"}
    @{Sub="Special Logon"; S="1"; F="0"; D="Special Logon"}
    @{Sub="User Account Management"; S="1"; F="1"; D="User Account Management"}
    @{Sub="Sensitive Privilege Use"; S="1"; F="1"; D="Sensitive Privilege Use"}
    @{Sub="Process Creation"; S="1"; F="0"; D="Process Creation"}
)
foreach ($C in $AuditCategories) {
    try {
        Set-GPRegistryValue -Name $GpoName -Key "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Audit" `
...
# Restrict Security log Clear
Write-Host "[*] Restrict Security log Clear..." -NoNewline -ForegroundColor Cyan
try {
    Set-GPRegistryValue -Name $GpoName -Key "HKLM\SYSTEM\CurrentControlSet\Services\EventLog\Security" `
        -ValueName "RestrictGuestAccess" -Type DWord -Value 1 -ErrorAction Stop | Out-Null
    Write-Host "                  [SET]" -ForegroundColor Green
...
# Security log max size 1073741824
Write-Host "[*] Setting Security log max size to 1073741824..." -NoNewline -ForegroundColor Cyan
try {
    Set-GPRegistryValue -Name $GpoName -Key "HKLM\SYSTEM\CurrentControlSet\Services\EventLog\Security" `
        -ValueName "MaxSize" -Type DWord -Value 1073741824 -ErrorAction Stop | Out-Null
    Write-Host "  [SET]" -ForegroundColor Green
